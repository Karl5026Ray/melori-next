...
Set on Vercel Production (added 2026-07-21). Nothing in the Concert path is
cron-driven today, so the preflight treats this as advisory rather than
blocking — but every scheduled route in `vercel.json`
...
- **Rounds do not advance on their own.** `concert_battles.phase_ends_at` is
  written and the stage counts down against it, but no route or scheduled job
  transitions `round_active` → `round_intermission` → next round → `completed`.
  A battle's timer will reach zero and stay there. Closing this needs a
  `CRON_SECRET`-protected route registered in `vercel.json` plus the state
  transition itself, with `concert_battle_rounds` remaining the authority for
  outcomes.
