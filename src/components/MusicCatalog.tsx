...
// The one catalog grid. `items` is the unified list — admin-curated releases
// and artist self-uploads mixed together, filtered and sorted as one body of
// work. There is deliberately no "source" filter: an artist upload is a
// catalog item like any other.
export default function MusicCatalog({
  ...
  // instead — used by the /music page so the page owns a single search input.
  ...
  const genres = useMemo(() => {
    // Merge the canonical list with genres actually present on items,
    // deduped case-insensitively (first spelling wins), then sorted.
    const byKey = new Map<string, string>();
    for (const g of [...CANONICAL_GENRES, ...items.map((r) => r.genre)]) {
      if (!g) continue;
  ...
  // "EP" counts as an album for the tab filter — it always did, since the tab
  // compared release_type directly and EPs simply matched neither pill.
  const albumCount = useMemo(
    () => items.filter((r) => r.release_type === "album").length,
    [items],
  );
  const singleCount = useMemo(
    () => items.filter((r) => r.release_type === "single").length,
    [items],
  );
