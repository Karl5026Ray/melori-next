...
// RoomScreen — the live room itself, shared by both room routes.
//
// This used to BE /social/spaces/[spaceId]/page.tsx, which is why Cinema and
// Spaces felt like one product: a Cinema room was literally the Spaces URL
// with a different `room_format`. The two formats now have their own routes
// (/social/spaces/[spaceId] and /social/cinema/[roomId]) and both render this
// component, so they can look and behave like separate products while sharing
// the room engine underneath — roles, the raise-hand queue, moderation, bans
// and teardown, none of which anyone wants two copies of.
//
// The route owns the URL and the format guard; this component owns the room.
// It takes spaceId as a prop rather than reading useParams(), because the two
// routes name their dynamic segment differently.
...
export function RoomScreen({ spaceId }: RoomScreenProps) {
