...
//
// This module is pure and client-safe (no server-only imports).
...
if (t === "admin") return "artist";
if (t === "artist") return "artist";
if (t === "superfan") return "superfan";
return "free";
}

// Active = status 'active' (missing treated as active) AND no expiry or future expiry.
...
// Comp access: internal test, complimentary, or billing-exempt profiles.
...
// "Superfan or better" — admins and comp accounts always qualify; otherwise active AND
// tier in ['superfan', 'artist'].
...
// Studio access — admins and comp always qualify; otherwise active AND tier === 'artist'.
