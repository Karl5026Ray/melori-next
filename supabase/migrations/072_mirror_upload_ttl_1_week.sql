-- 072_mirror_upload_ttl_1_week.sql
-- =============================================================================
...
--
-- WHY 072 AND NOT A NUMBER NEAR ITS APPLY DATE: the numeric prefix is this
-- repo's apply-order contract (see scripts/migration-prefix.test.ts and the
-- 021 / 048 / 054 collisions behind it). Squeezing this in beside an existing
-- number would either collide or renumber history, so it takes the next free
-- slot instead. Order is safe: this file replaces the set_feed_expiry()
-- trigger while 070 replaces seed_mirror_feed() -- different objects, both
-- idempotent, no dependency in either direction.
--
